...
for input_file in input_files:
    ...
    output_file_igem = f"igem_{input_file}"
    ...
    with open(os.path.join(output_directory, output_file), "w") as file:
        ...
        # Get Gene and Term from IGEM > OMICS
        # Convert split_data into a list of dictionaries
        data_dict = [{'chromosome': item[2], 'position': item[3]} for item in split_data]

        # Create a DataFrame from the list of dictionaries
        df = pd.DataFrame(data_dict)
        # Replace 'not found' with 0
        df['position'] = df['position'].replace('not found', 0)
        df['chromosome'] = df['chromosome'].replace('not found', 0)
        for n in range(23):
            df['chromosome'] = df['chromosome'].replace(f'chr{n}', n)
        df['chromosome'] = df['chromosome'].replace('chrx', 23)
        df['chromosome'] = df['chromosome'].replace('chry', 24)

        df_results = ge.filter.search_gene_map_data(input_df=df)
        df_results.to_csv(str(output_directory + "/" + output_file_igem))
